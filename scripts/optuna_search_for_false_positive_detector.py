...
save_dir = 'val_nms030_tta7_5models_hard_prob'
...
val_images = [line.rstrip() for line in open('input/val_images.list').readlines()]
count = len(val_images)

with open('input/full_train_without_empty.list') as ft:
    train_without_empty_set = set([line.rstrip() for line in ft.readlines()])
...
def check_box(boxlist, size, prob_list):
    ...
    # check broken_box, inside_box, overlap_rate
...
def gen_info(prob, label, bbox, box_score, broken, overlap_rate, nearest_dict, new_boxlist, size):
    w_list = []
    h_list = []
    area_list = []
    x_point_list = []
    ...
    current_info = {
        ...
        'area_std_rate': width * height / std_area,
        ...
        'nearest_area_std_rate': nearest_width * nearest_height / std_area,
        ...
        'nearest2_area_std_rate': nearest2_width * nearest2_height / std_area,
        ...
        'nearest3_area_std_rate': nearest3_width * nearest3_height / std_area,
        ...
        'nearest4_area_std_rate': nearest4_width * nearest4_height / std_area,
        ...
        'nearest5_area_std_rate': nearest5_width * nearest5_height / std_area,
        ...
        'x_std_rate': x_center / std_x if std_x else 0,
        'y_std_rate': y_center / std_y if std_y else 0,
...
for target_index in range(0,count):
    ...
    if skip_empty_page:
        if not image_id in train_without_empty_set:
            continue

    train = False
    for book_name in target_books:
        if book_name in image_id:
            train = True
    if not train:
        continue
    ...
    for i, (prob, label, bbox, box_score, broken, overlap_rate, nearest_dict) in \
        ...
        sub_str, current_info = gen_info(prob, label, bbox, box_score, broken, overlap_rate, nearest_dict, new_boxlist, size)
        sub_list.append(sub_str)
        sub_info.append(current_info)

    sub_labels = ' '.join(sub_list)

    preds_unused, _result_score = score_page(sub_labels, train_df[train_df.image_id == image_id].values[0][1])
    print(".", end='')

    if gather_info:
        preds_unused = preds_unused.tolist()
        preds_unused_master.extend(preds_unused)
        sub_info_master.extend(sub_info)

sub_info = []
for sub, pred in zip(sub_info_master, preds_unused_master):
    sub['target'] = int(pred)
    sub_info.append(sub)

df = pd.DataFrame(sub_info)

df.to_feather('val_nms030_tta7_5models_hard_prob.feather')
# df = pd.read_feather('val_nms030_tta7_5models_hard_prob.feather')
...
X_train, X_test, y_train, y_test = train_test_split(tmp_df, target, test_size=0.20)
