...
def im_detect_bbox_aug(model, images, device):
    ...
    for scale in cfg.TEST.BBOX_AUG.SCALES:
        ...
        if cfg.TEST.BBOX_AUG.GRAYSCALE:
            boxlists_scl_gray = im_detect_bbox_scale(
                model, images, scale, max_size, device, grayscale=True
            )
            add_preds_t(boxlists_scl_gray)
...
def im_detect_bbox_grayscale(model, images, target_scale, target_max_size, device):
    """
    Performs bbox detection on the grayscale image.
    """
    transform = TT.Compose([
        T.Resize(target_scale, target_max_size),
...
def im_detect_bbox_scale(model, images, target_scale, target_max_size, device, hflip=False, grayscale=False):
    """
    Computes bbox detections at the given scale.
    Returns predictions in the scaled image space.
    """
    # not implement hflip and grayscale yet
    if hflip:
        boxlists_scl = im_detect_bbox_hflip(model, images, target_scale, target_max_size, device)
    elif grayscale:
        boxlists_scl = im_detect_bbox_grayscale(model, images, target_scale, target_max_size, device)
    else:
